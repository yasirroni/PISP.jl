...
const ISP2026_BUS_ALIASES = Dict("MEL" => "VIC", "WNV" => "VIC", "SEV" => "VIC", "NSA" => "CSA")
...
function canonicalize_line_invoptions(raw::DataFrame, report::ISPValidationReport)
    ...
    for row_number in 1:nrow(raw)
        ...
        bus_from_id = ParseISP._ispdata_bus_id(bus_from)
        bus_to_id = ParseISP._ispdata_bus_id(bus_to)
        ...
        fwd = ParseISP.flow2num(raw[row_number, fwd_col])
        rev = ParseISP.flow2num(raw[row_number, rev_col])
        cost = ParseISP.inv2num(split(string(raw[row_number, cost_col]), ['(', '\n']))
        lead = ParseISP.lead2year(split(string(raw[row_number, lead_col]), ['(', '\n'])[1])
