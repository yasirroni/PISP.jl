...
if !isdefined(@__MODULE__, :NEMAREAS)
    include(joinpath(dirname(@__DIR__), "parameters", "general.jl"))
end
...
function ev_read_non_empty_rows(workbook_path::AbstractString, sheet_name::AbstractString, range_ref::AbstractString)
    raw_sheet = XLSX.readdata(workbook_path, ParseISP.resolve_sheet_name(workbook_path, sheet_name), range_ref)
